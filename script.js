// Check if the favourites meal array exists in local storage, if not, create an empty array
...
// Function to display all meals cards in main according to search input value
    ...
    meals.then((data) => {
        if (data.meals) {
            data.meals.forEach((element) => {
                ...
                if (isFav) {
                    // HTML template for favourite meals
                    html += `
                ...
                `;
                    ...
                    // HTML template for non-favourite meals
                    html += `
                <div id="card" class="card mb-3" style="width: 20rem;">
                    <img src="${element.strMealThumb}" class="card-img-top" alt="...">
                    <div class="card-body">
            ...
            // Displaying a 404 message if no meals are found
            html += `
        ...
        // Inserting HTML into the main container
        document.getElementById("main").innerHTML = html;
    });
}

// Function to display full meal details in main
async function showMealDetails(id) {
    let url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=";
    let html = "";
    await fetchMealsFromApi(url, id).then((data) => {
        // Generating HTML for meal details
        html += `
...
// Function to display all favourite meals in favourites body
async function showFavMealList() {
    let arr = JSON.parse(localStorage.getItem("favouritesList"));
    let url = "https://www.themealdb.com/api/json/v1/1/lookup.php?i=";
    let html = "";
    if (arr.length == 0) {
        // Displaying a message if no meals are added to favourites
        html += `
            <div class="page-wrap d-flex flex-row align-items-center">
                <div class="container">
                    <div class="row justify-content-center">
        ...
        for (let index = 0; index < arr.length; index++) {
            await fetchMealsFromApi(url, arr[index]).then((data) => {
                // Generating HTML for each favourite meal
                html += `
                <div id="card" class="card mb-3" style="width: 20rem;">
    ...
    // Inserting HTML into the favourites body container
    document.getElementById("favourites-body").innerHTML = html;
}

// Function to add and remove meals to/from favourites list
